...
var SailsPostgresDiskAdapter = require('sails-postgresql');
	...
	Waterline.start(
		{
			adapters: {
				...
				'sails-postgresql': SailsPostgresDiskAdapter,
			},
		...
		function whenWaterlineIsReady(err, orm) {
			if (err) {
				return done(
					new Error('Could not start up Waterline ORM: ' + err.stack),
				);
			} //--•
			...
			(function (proceed) {
				...
				Pet.createEach([
					...
					.exec(function (err, pets) {
						if (err) {
							return proceed(
								new Error('Failed to create new pets: ' + err.stack),
							);
						}
						...
						User.create({
							...
							if (err) {
								return proceed(
									new Error('Failed to create new user: ' + err.stack),
								);
							}
							...
							User.stream()
								...
								.exec(function afterwards(err) {
									if (err) {
										return proceed(
											new Error(
												'Unexpected error occurred while streaming users:',
												err.stack,
											),
										);
									}
				...
				if (err) {
					Waterline.stop(orm, function (secondaryErr) {
						if (secondaryErr) {
							handleLog();
							handleLog(
								'An error occurred, and then, when trying to shut down the ORM gracefully, THAT failed too!',
							);
							handleLog('More on the original error in just a while.');
							handleLog(
								"But first, here's the secondary error that was encountered while trying to shut down the ORM:\n",
								secondaryErr,
							);
							handleLog(
								'... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ... ',
				...
				Waterline.stop(orm, function (secondaryErr) {
					if (secondaryErr) {
						return done(
							new Error(
								'Everything else went fine, but then when attempting to shut down the ORM gracefully, something went wrong!  Details:' +
									secondaryErr.stack,
							),
						);
					}
