...
export type RetTotal = Awaited<ReturnType<typeof asd>>;

const order = 'DESC';
const zTotalInput = tDateFilter.extend({isCalculated: z.boolean()}).partial();
...
export function dashboardRouters() {
	return router({
		...
		total: procedure.input(zTotalInput).query(({ctx, input}) => {
			return checkCredentialV2(ctx, () => asd(input));
		}),

		totalList: procedure.input(zTotalInput.array()).query(({ctx, input}) => {
...
async function getData(input: Partial<TDateFilter>) {
	...
	const outQty = oOut.findAll({
		...
		order: [[col('oStock.unit'), order]],
		group: [col('oStock.unit'), col('oStock.harga'), col('oStock.oItem.harga')],
		include: [
			{
				model: oStock,
				attributes: [],
...
async function asd(input: z.infer<typeof zTotalInput>) {
	const data = await getData(input);
	const parsedData = parseData(data);
	const calculateData = entries(parsedData).reduce((ret, [key, item]) => {
		if (!ret[key]) ret[key] = {className: item.className, value: 0};

		item.data.forEach(e => {
			ret[key].value += parseFloat(findValue(e, input.isCalculated).toString());
		});

		return ret;
	}, {} as Record<keyof ReturnType<typeof parseData>, {className: string; value: number}>);
