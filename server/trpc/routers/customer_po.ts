...
export const customer_po_get = procedure
	.input(
		z.object({
			type: z.string(),
		}),
	...
	.query(async ({ctx: {req, res}}) => {
		return checkCredentialV2(req, res, async () => {
			const allPO = (await OrmCustomerPO.findAll({raw: true})) as TCustomerPO[];
...
export const customer_po_insert = procedure
	.input(tCustomerPO)
...
export const customer_po_update = procedure
	.input(tCustomerPO)
	.mutation(async ({input, ctx: {req, res}}) => {
		return checkCredentialV2(req, res, async () => {
...
export const customer_po_delete = procedure
	.input(tCustomerPO)
	.mutation(async ({input, ctx: {req, res}}) => {
		return checkCredentialV2(req, res, async () => {
